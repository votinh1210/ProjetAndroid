...
public class ModbusLocator {
    ...
    public ModbusLocator(int slaveId, int range, int offset, byte bit) {
        ...
        if (range != RegisterRange.HOLDING_REGISTER && range != RegisterRange.INPUT_REGISTER)
            throw new ModbusIdException("Bits demandes  ne peuvent être effectuées à partir des registres d'entrée.");
    }
    ...
    private void validate() {
        ...
        if (dataType != DataType.BINARY && (range == RegisterRange.COIL_STATUS || range == RegisterRange.INPUT_STATUS))
            throw new IllegalDataTypeException("Seules les valeurs binaires peuvent être lus à partir des gammes de ``Coil` et d'entrée.");
    ...
    /**
     * Convertit les données d'une valeur java dans le tableau de bytes 
     * en fonction du type de données de ce localisateur. Cette méthode ne gère pas le type binaire.
     * 
     * @param value
     * @param dataType
     * @return the converted data
    ...
    public static short[] valueToShorts(Number value, int dataType) {
        ...
        throw new RuntimeException("Type de données non pris en charge : " + dataType);
    }
