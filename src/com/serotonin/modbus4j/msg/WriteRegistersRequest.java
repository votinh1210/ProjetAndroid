...
public class WriteRegistersRequest extends ModbusRequest {
	...
	public WriteRegistersRequest(int slaveId, int startOffset, short[] sdata)
			throws ModbusTransportException {
		super(slaveId);

		ModbusUtils.validateOffset(startOffset);
		if (sdata.length < 1
				|| sdata.length > ModbusUtils.MAX_WRITE_REGISTER_COUNT)
			throw new ModbusTransportException(
					"Nombre de registres non valide : " + sdata.length, slaveId);
