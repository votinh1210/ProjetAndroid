...
abstract public class ModbusRequest extends ModbusMessage {
	public static ModbusRequest createModbusRequest(ByteQueue queue)
		...
		if (functionCode == FunctionCode.READ_COILS)
			...
			request = new ExceptionRequest(slaveId, functionCode,
	...
	public ModbusResponse handle(ProcessImage processImage)
			throws ModbusTransportException {
		try {
			try {
	...
	abstract ModbusResponse handleImpl(ProcessImage processImage)
			throws ModbusTransportException;

	abstract protected void readRequest(ByteQueue queue);

	ModbusResponse handleException(byte exceptionCode)
			throws ModbusTransportException {
	...
	abstract ModbusResponse getResponseInstance(int slaveId)
			throws ModbusTransportException;
